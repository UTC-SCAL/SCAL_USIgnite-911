...
def fitting_loops(X, Y, folder):
    ...
    for i in range(0, 50):
        file = folder + str(datetime.date.today()) + "GridFixedAverageHolder.csv"
        ...
        if exists(folder + 'model_ORI_GridFix_Hour.h5'):
            model.load_weights(folder + 'model_ORI_GridFix_Hour.h5')
        ...
        model.save_weights(folder + 'model_ORI_GridFix_Hour.h5')
...
dataset = shuffle(dataset)
folder = '../Graphs & Images/ResultsFromGridFixTesting/Seventh Test - Full Data with Hour/'
