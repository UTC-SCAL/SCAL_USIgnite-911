...
forecast0MMR$Latitude = forecast0stand$Latitude
forecast0MMR$Longitude = forecast0stand$Longitude

a=which(forecast0MMR$Prediction==1)
length(a)
forecast0MMR=forecast0MMR[a,c('Latitude','Longitude', 'Probability')]

b=which(forecast0stand$Prediction==1)
forecast0stand=forecast0stand[b,c('Latitude','Longitude', 'Probability')]
length(b)

library(reshape)
zz <- melt(list(MMR=forecast0MMR,stand=forecast0stand,accident=accidents), id.vars=c("Latitude", "Longitude", "Probability"))
head(zz)

thresh = which(zz$Probability>=0.95)
zz=zz[thresh,]


png("../Graphs & Images/R Maps/April03_noon_.95thresh.png", width=6,height=6,units='in', res=800)
lats<-c(34.99, 35.45)
lons<-c(-85.4,-85.05)
par(mar = rep(0, 4))
bb<-make_bbox(lon=lons,lat=lats, f=0.05)
calc_zoom(bb)
cda<-get_map(bb,zoom=13,maptype="roadmap", source='google')
ggmap(cda) +  geom_point(aes(x=zz$Longitude, y=zz$Latitude, color=zz$L1), alpha=.25, data=zz) + guides( guide = "none" ) + scale_color_manual(values = c("black","red","dodgerblue"),  labels = c("Accident","MMR","Standard"), name = "Occurrences")  + scale_alpha_manual(c(1.5,.7,.15)) + ggtitle("Accidents Predicted at noon .95 threshold")
dev.off()
```
