...
title = "/Users/peteway/Downloads/PredictionsForNewTests/TS_50-50 Split_Test3Forecast.csv"
...
model  = str_split(str_split(title, "/")[[1]][6], "Forecast.csv")[[1]][1]
model = gsub("_", " ", model)
...
for(frame in dayframes){
  ...
  savename = paste("/Users/peteway/Downloads/PredictionsForNewTests/Viz/TESTINGPredictionMapfor_",titledate, "_",model,"_DayFrame",frame,".png",sep="")
  ...
  blocksdayframe = merge(blockstest, dayframe, by="Grid_Num")
  # thresh = ifelse(frame==1, .62, ifelse(frame==2, .72, ifelse(frame==3, .92,.87)) )
  thresh = .5
  ...
  map = (ggmap(cda) + coord_sf(crs = st_crs(blockstest)) + geom_sf(data=blocksdayframe, aes(fill = blocksdayframe$Probability), alpha=.5, inherit.aes = FALSE) + scale_fill_gradient(low = "blue", high = "red","Probability", limits=c(.5,1)) + xlab("Longitude") + ylab("Latitude") + geom_sf(data=accframe, color = 'black', shape = accframe$Shape,size=3,fill='black', inherit.aes = FALSE) + ggtitle(paste("Accident Prediction",titledate,model,"Dayframe",frame, sep=" ")))
  # + transparent  + 
  print(map)
    dev.off()
}
