...
files <-
    list.files(pattern = "Week Hex Confusion Matrix Quantities.csv", path=path, recursive = TRUE, full.names = TRUE) 
...
write_csv(means, "/Users/peteway/Documents/GitHub/SCAL_USIgnite-911/Excel & CSV Sheets/Forecasts/AveragePerformanceforHexwith5Tests.csv")
```
...
path = paste("/Users/peteway/Documents/GitHub/SCAL_USIgnite-911/Excel & CSV Sheets/Forecasts/",date,"/Forecast",sep="")
files <-
    list.files(pattern = "Test2Forecast.csv", path=path, recursive = TRUE, full.names = TRUE) 
...
if(length(forecast$Grid_Num) != 0)
  {
    ...
    test = merge(test, for_grids, by=c("Grid_Num","DayFrame"), all.x=TRUE )
    test[is.na(test)] <- 0
    filename = paste("/Users/peteway/Documents/GitHub/SCAL_USIgnite-911/Excel & CSV Sheets/Forecasts/",month,"-",day,"-",year,"/Confusion Matrix/",modeltypes[num],"Test.csv", sep="")
    write.csv(test, filename)
    print(clippedmodeltype)
}
num = num+1
}
```
