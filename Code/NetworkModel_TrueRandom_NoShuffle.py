...
def fitting_loops(dataset, testing, folder):
    X_train = dataset.ix[:, 1:(len(dataset.columns) + 1)].values
    y_train = training.Accident.values

    X_test = testing.ix[:, 1:(len(dataset.columns) + 1)].values
    y_test = testing.Accident.values
    ...
    for i in range(0, 50):
        file = folder + str(datetime.date.today()) + "GridFixedAverageHolder_NoShuffle.csv"
        ...
        if exists(folder + 'model_TrueRandom_NoShuffle.h5'):
            model.load_weights(folder + "model_TrueRandom_NoShuffle.h5")
        ...
        model.save_weights(folder + "model_ORI_GridFix_NoShuffle.h5")
...
#           1. Load Data
training = pandas.read_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/True Randoms/True Random Data Non-Shuffle Random Data Non-Shuffle Training.csv", sep=",")
testing = pandas.read_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/True Randoms/True Random Data Non-Shuffle Random Data Non-Shuffle Testing.csv", sep=",")
...
folder = "../Graphs & Images/ResultsFromTrueRandomTesting/Second Test - No Shuffle/"
#           Shuffling if needed. 
...
fitting_loops(training, testing, folder)
