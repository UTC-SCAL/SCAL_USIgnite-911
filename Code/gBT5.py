...
negatives = pandas.read_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/NS GridFix Master List 4_1.csv",index_col=False)
...
for i, info in enumerate(negatives.values):
    ...
    if no_matches is True:
        ...
        neg_loc += 1
    if i % 500 == 0:
        negative_samples.to_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/NS Master List 4.csv", index=False)
negative_samples.to_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/NS  Master List 4.csv", index=False)
exit()
