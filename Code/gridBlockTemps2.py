...
for i in range(938, 1100):
    ...
    for k, info in enumerate(grid_section4.values):
        print(k)
        # All variables are blank-of-accident, thus year is yoa.
        doa = grid_section4.Date.values[k]
        yoa = int(doa.split('-')[0])
        moa = int(doa.split('-')[1])
        ...
        for j, value2 in enumerate(forecastcall.daily):
            ...
            grid_section4[col_name].values[k] = str(temp_max) + "|" + str(temp_min) + "|" + str(temp_avg)
    grid_section4.to_csv("../Excel & CSV Sheets/Grid Layout Test Files/Grid Blocks Section 4.csv")
