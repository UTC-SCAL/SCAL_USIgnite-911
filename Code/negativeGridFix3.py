    ...
    for j, values in enumerate(calldata.values):
            ...
            for n, checks in enumerate(compare.values):  # Iterates through copy_calldata checking for a match with i
                if n == j:
                    ...
                    if copy_calldata.Hour.values[0] == compare.Hour.values[n] and \
                                copy_calldata.Date.values[0] is compare.Date.values[n] and \
                                    copy_calldata.Grid_Block.values[0] is compare.Grid_Block.values[n]:
                        no_matches = False
                        break
        ...
        if j % 500 == 0:
            negative_samples.to_csv\
                ("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/NS GridFix Master List 3.csv")

    negative_samples.to_csv\
        ("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/NS GridFix Master List 3.csv")


accidents = pandas.read_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/GOD Section 3.csv")
