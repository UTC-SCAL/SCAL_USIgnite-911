...
def add_data(calldata, save_name):
    ...
    calldata.Weekday = calldata.Weekday.astype(int)
...
def append_data(calldata):
    ...
    og_calldata = pandas.read_excel(
        folderpath + "Excel & CSV Sheets/New Data Files/New Accident Data.csv")
...
def get_hour_negatives(calldata):
    ...
    negative_samples = pandas.read_excel(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/2017+2018 Data/NegativeSamples.xlsx")
    ...
    for i, info in enumerate(calldata.values):
        ...
        for k, checks in enumerate(calldata.values):  # Iterates through calldata checking for a match with i
            if calldata.Hour.values[k] == calldata.Hour.values[i] and \
                            calldata.Date.values[k] is calldata.Date.values[i]:
                # If match, skip
                pass
            else:
    ...
    calldata = calldata.iloc[::-1]
    og_calldata = pandas.read_excel(
        folderpath + "Excel & CSV Sheets/New Data Files/New Negative Samples (Hour).csv")
    frames = [og_calldata, calldata]
    results = pandas.concat(frames)
    header_list = (
...
def get_date_negatives(calldata):
    ...
    # This file needs to be updated every day #
    day_holder2017 = pandas.read_excel(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/2017+2018 Data/Day Holder 2017.xlsx")
    ...
    calldata = calldata.iloc[::-1]
    og_calldata = pandas.read_excel(
        folderpath + "Excel & CSV Sheets/New Data Files/New Negative Samples (Date).csv")
    frames = [og_calldata, calldata]
    results = pandas.concat(frames)
    header_list = ("Accident", 'Latitude', 'Longitude', 'Date', 'Time', 'Address', "Route", "Log_Mile", 'City', 'Event',
                   'Conditions', "EventBefore", "ConditionBefore", 'Hour', 'Temperature', "Temp_Max", "Temp_Min",
...
def main():
    ...
    # calldata = pandas.read_csv(file, sep=",",
    #         dtype={"Accident": int, "Latitude": float, "Longitude": float, 'Date': datetime, 'Time': datetime.time,
    ...
    calldata = pandas.read_excel(folderpath + "Excel & CSV Sheets/2018 Data/"+ dayname_xlsx + ".xlsx",
            dtypes={"Accident": int, "Latitude": float, "Longitude": float, 'Date': datetime, 'Time': datetime.time,
                    "Address": str, "Route": str, "Log_Mile": float, "City": str, 'Event': str,'Conditions': str,
                    "EventBefore": str, "ConditionBefore": str, 'Hour': int, 'Temperature': float, "Temp_Max": float,
                    "Temp_Min": float, "Monthly_Avg_Temp": float, "Daily_Avg_Temp": str, "Relative_Temp": float,
                    "Dewpoint": float, 'Humidity': float, "Month": int, "Weekday": int, 'Visibility': float,
