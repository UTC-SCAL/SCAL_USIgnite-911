...
def get_road_geometrics(calldata):
    ...
    descriptions = pandas.read_csv(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/ETRIMS/Roadway_Description_County_HAMILTON.csv",
        sep=",")
            ...
            if calldata.Route.values[k] == descriptions.ID_NUMBER.values[n]:
                if descriptions.ELM.values[n] > calldata.Log_Mile.values[k] > descriptions.BLM.values[n]:
                    ...
                    if descriptions.Feature_Type[n] == 'ACCELERATION/DECELERATION LANE':
                        calldata.Turn_Lane.values[k] = 1
                    else:
                        calldata.Turn_Lane.values[k] = 0
    return calldata
...
def add_data(calldata, save_name):
    ...
    traffic = pandas.read_csv(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/ETRIMS/Traffic_Count.csv",
        sep=",")
    print("Getting Normal Road Geometrics")
    for k, info in enumerate(calldata.values):
        print(k)
        for i, value in enumerate(geometrics.values):
            if calldata.Route.values[k] == geometrics.ID_NUMBER.values[i]:
...
def get_hour_negatives(calldata):
    ...
    for i, info in enumerate(calldata.values):
        ...
        for n, checks in enumerate(calldata.values):  # Iterates through calldata checking for a match with i
            if calldata.Hour.values[n] == calldata.Hour.values[i] and \
                ...
                neg_loc = neg_loc + 1
                break
    print("Getting Negative Hour Samples")
    driver = webdriver.Firefox(executable_path=r"/home/admin/PycharmProjects/RolandProjects/geckodriver")
    driver.get("https://e-trims.tdot.tn.gov/Account/Logon")
    ...
    for i, info in enumerate(negative_samples.values):
        ...
        # print("milepoint={}, routeid={}".format(milepoint, routeid))
        negative_samples.Route.values[i] = routeid
        negative_samples.Log_Mile.values[i] = milepoint
    ...
    traffic = pandas.read_csv(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/ETRIMS/Traffic_Count.csv",
        sep=",")
    print("Getting Negative Samples by Hour Road Geometrics")
    ...
    negative_samples.Precipitation_Type = negative_samples.Precipitation_Type.astype(str)
    negative_samples.Precipitation_Intensity = negative_samples.Precipitation_Intensity.astype(float)
    negative_samples.Precip_Intensity_Max = negative_samples.Precip_Intensity_Max.astype(float)
    negative_samples.Temp_Max = negative_samples.Temp_Max.astype(float)
    negative_samples.Temp_Min = negative_samples.Temp_Min.astype(float)
    negative_samples.Precip_Intensity_Time = negative_samples.Precip_Intensity_Time.astype(datetime)
    ...
    # Format the latitude and longitude values into the appropriate formats, if they need to be formatted
    for k, info in enumerate(negative_samples.values):
        if negative_samples.Latitude.values[k] > 40:
            negative_samples.Latitude.values[k] = (negative_samples.Latitude.values[k] / 1000000)
            negative_samples.Longitude.values[k] = (negative_samples.Longitude.values[k] / -1000000)
    ...
    print("Adding in DarkSky Weather for Negative Date Samples")
    # Iterate through negative_samples and assign weather data for each incident
    for k, info in enumerate(negative_samples.values):
        print(k)
        # All variables are blank-of-accident, thus year is yoa.
        hoa = int(negative_samples.Hour.values[k])
        toa = negative_samples.Time.values[k]
        mioa = int(toa.split(':')[1])
        soa = int(toa.split(':')[2])
        doa = negative_samples.Date.values[k]
        yoa = int(doa.split('-')[0])
        moa = int(doa.split('-')[1])
        dayoa = int(doa.split('-')[2])
        lat = negative_samples.Latitude.values[k]
        long = negative_samples.Longitude.values[k]
        ...
        if hoa == 0 and dayoa == 1:  # If 1/1, get weather data from 12/31, reduce year by 1
            if moa == 1:
                ...
                try:
                    forecastcall = forecast(*call, time=t)
                    for i, value in enumerate(forecastcall.hourly):
                        negative_samples.EventBefore.values[k] = value.icon
                        negative_samples.ConditionBefore.values[k] = value.summary
                except:
                    print("Error in finding previous hour")
            elif moa == 2:  # If 2/1, get weather data from 1/31, same year
                new_hoa = 23
        ...
        try:
            ...
            for i, value in enumerate(forecastcall.hourly):
                # Retrieving weather for previous weather
                if i == hoa:
                    negative_samples.Temperature.values[k] = value.temperature
                    negative_samples.Dewpoint.values[k] = value.dewPoint
                    negative_samples.Event.values[k] = value.icon
                    negative_samples.Humidity.values[k] = value.humidity
                    negative_samples.Month.values[k] = moa
                    negative_samples.Visibility.values[k] = value.visibility
                    negative_samples.Conditions.values[k] = value.summary
        except:
            print("Hourly Lookup Failed")
            # try:
            # Daily data, which requires individual try/except statements, otherwise the code crashes for some reason
        for j, value2 in enumerate(forecastcall.daily):
            try:
                negative_samples.Precipitation_Type.values[k] = value2.precipType
            except:
                negative_samples.Precipitation_Type.values[k] = "NA"
            try:
                negative_samples.Precipitation_Intensity.values[k] = value2.precipIntensity
            except:
                negative_samples.Precipitation_Intensity.values[k] = -1000
            try:
                negative_samples.Precip_Intensity_Max.values[k] = value2.precipIntensityMax
            except:
                negative_samples.Precip_Intensity_Max.values[k] = -1000
            try:
                negative_samples.Precip_Intensity_Time.values[k] = value2.precipIntensityMaxTime
            except:
                negative_samples.Precip_Intensity_Time.values[k] = -1000
            try:
                negative_samples.Temp_Max.values[k] = value2.temperatureMax
            except:
                negative_samples.Temp_Max.values[k] = -1000
            try:
                negative_samples.Temp_Min.values[k] = value2.temperatureMin
            except:
                negative_samples.Temp_Min.values[k] = -1000
            try:
                negative_samples.Cloud_Coverage.values[k] = value2.cloudCover
            except:
                negative_samples.Cloud_Coverage.values[k] = -1000
    ...
    for i, value in enumerate(negative_samples.values):
        ...
        if "cloudy" in negative_samples.Event.values[i] or "cloudy" in negative_samples.Conditions.values[i] \
                ...
                or "Overcast" in negative_samples.Event.values[i] or "Overcast" in negative_samples.Conditions.values[
            i]:
        ...
        if "rain" in negative_samples.EventBefore.values[i] or "rain" in negative_samples.ConditionBefore.values[i] \
                or "Rain" in negative_samples.EventBefore.values[i] or "Rain" in \
                negative_samples.ConditionBefore.values[i]:
...
def get_date_negatives(calldata):
    ...
    for i, info in enumerate(calldata.values):
        ...
        for k, checks in enumerate(calldata.values):  # Iterates through calldata checking for a match with i
            if calldata.Date.values[k] == calldata.Date.values[i]:
                ...
                neg_loc = neg_loc + 1
                break
    print("Getting Negative Date Routes")
    driver = webdriver.Firefox(executable_path=r"/home/admin/PycharmProjects/RolandProjects/geckodriver")
    driver.get("https://e-trims.tdot.tn.gov/Account/Logon")
    ...
    traffic = pandas.read_csv(
        "/home/admin/PycharmProjects/RolandProjects/Excel & CSV Sheets/ETRIMS/Traffic_Count.csv",
        sep=",")
    print("Getting Negative Samples by Date Road Geometrics")
    for k, info in enumerate(negative_samples.values):
        print(k)
        for i, value in enumerate(geometrics.values):
            if negative_samples.Route.values[k] == geometrics.ID_NUMBER.values[i]:
    ...
    print("Adding in DarkSky Weather for Negative Hour Samples")
    # Iterate through negative_samples and assign weather data for each incident
    for k, info in enumerate(negative_samples.values):
        print(k)
        # All variables are blank-of-accident, thus year is yoa.
    ...
    for i, value in enumerate(negative_samples.values):
        print(i)
        if "clear" in negative_samples.Event.values[i] or "clear" in negative_samples.Conditions.values[
            i] \
                or "Clear" in negative_samples.Event.values[i] or "Clear" in \
        ...
        if "rain" in negative_samples.Event.values[i] or "rain" in negative_samples.Conditions.values[i] \
                or "Rain" in negative_samples.Event.values[i] or "Rain" in \
                negative_samples.Conditions.values[i] \
        ...
        if "snow" in negative_samples.Event.values[i] or "snow" in negative_samples.Conditions.values[i] \
                or "Snow" in negative_samples.Event.values[i] or "Snow" in \
                negative_samples.Conditions.values[i]:
            negative_samples.Snow.values[i] = 1
        else:
            negative_samples.Snow.values[i] = 0

        if "cloudy" in negative_samples.Event.values[i] or "cloudy" in \
                negative_samples.Conditions.values[i] \
                or "Cloudy" in negative_samples.Event.values[i] or "Cloudy" in \
                negative_samples.Conditions.values[i] \
                or "overcast" in negative_samples.Event.values[i] or "overcast" in \
                negative_samples.Conditions.values[i] \
                or "Overcast" in negative_samples.Event.values[i] or "Overcast" in \
                negative_samples.Conditions.values[
                    i]:
            negative_samples.Cloudy.values[i] = 1
        else:
            negative_samples.Cloudy.values[i] = 0
        ...
        if "fog" in negative_samples.Event.values[i] or "foggy" in negative_samples.Conditions.values[i] \
                or "Fog" in negative_samples.Event.values[i] or "Foggy" in \
                negative_samples.Conditions.values[i]:
            negative_samples.Fog.values[i] = 1
        else:
            negative_samples.Fog.values[i] = 0
        if "rain" in negative_samples.EventBefore.values[i] or "rain" in \
                negative_samples.ConditionBefore.values[i] \
                or "Rain" in negative_samples.EventBefore.values[i] or "Rain" in \
                negative_samples.ConditionBefore.values[i]:
            negative_samples.RainBefore.values[i] = 1
...
def main():
    ...
    file = folderpath + "Excel & CSV Sheets/2018 Data/DailyReports/911_Reports_for_2018-09-27.csv"
    ...
    # calldata = pandas.read_csv(file, sep=",",
    #         dtype={"Accident": int, "Problem": str, "Latitude": float, "Longitude": float, 'Date': datetime, 'Time': datetime.time,
    #                 "Address": str, "Route": str, "Log_Mile": float, "City": str, 'Event': str,'Conditions': str,
    #                 "EventBefore": str, "ConditionBefore": str, 'Hour': int, 'Temperature': float, "Temp_Max": float,
    #                 "Temp_Min": float, "Monthly_Avg_Temp": float, "Daily_Avg_Temp": str, "Relative_Temp": float,
    #                 "Dewpoint": float, 'Humidity': float, "Month": int, "Weekday": int, 'Visibility': float,
    #                 "Cloud_Coverage": float, "Precipitation_Type": str, "Precipitation_Intensity": float,
    #                 "Precip_Intensity_Max": float, "Precip_Intensity_Time": float, "Clear": int, "Cloudy": int,
    #                 "Rain": int, "Fog": int, "Snow": int, "RainBefore": int, "Terrain": int, "Land_Use": int,
    #                 "Access_Control": int, "Illumination": int, "Operation": int, "Speed_Limit": int, "Thru_Lanes": int,
    #                 "Num_Lanes": int, "Ad_Sys": int, "Gov_Cont": int, "Func_Class": int, "AADT": int, "DHV": int,
    #                 "Pavement_Width": int, "Pavement_Type": str, "Turn_Lane": int})
    ...
    calldata = pandas.read_excel(folderpath + "Excel & CSV Sheets/2018 Data/"+ dayname_xlsx + ".xlsx",
            dtypes={"Accident": int, "Problem": str, "Latitude": float, "Longitude": float, 'Date': datetime, 'Time': datetime.time,
                    "Address": str, "Route": str, "Log_Mile": float, "City": str, 'Event': str,'Conditions': str,
                    "EventBefore": str, "ConditionBefore": str, 'Hour': int, 'Temperature': float, "Temp_Max": float,
                    "Temp_Min": float, "Monthly_Avg_Temp": float, "Daily_Avg_Temp": str, "Relative_Temp": float,
                    "Dewpoint": float, 'Humidity': float, "Month": int, "Weekday": int, 'Visibility': float,
    ...
    calldata = pandas.read_excel(folderpath + "Excel & CSV Sheets/2018 Data/" + dayname_xlsx + "_Dropped_Dupes.xlsx",
            dtypes={"Accident": int, "Problem": str, "Latitude": float, "Longitude": float, 'Date': datetime, 'Time': datetime.time,
                    "Address": str, "Route": str, "Log_Mile": float, "City": str, 'Event': str,'Conditions': str,
                    "EventBefore": str, "ConditionBefore": str, 'Hour': int, 'Temperature': float, "Temp_Max": float,
                    "Temp_Min": float, "Monthly_Avg_Temp": float, "Daily_Avg_Temp": str, "Relative_Temp": float,
    ...
    # calldata = get_road_geometrics(calldata)
