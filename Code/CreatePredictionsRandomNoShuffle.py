...
def predict_accidents(forecast, filename, modelpath):
    # dataset = pandas.read_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/True Randoms/True Random Data 2017+2018.csv", sep=",")
    # dataset = dataset.drop(['Unnamed: 0','Latitude','Longitude','Date','Time'],axis=1)
    dataset = pandas.read_csv(
        "../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/Randoms/GOD Random Full.csv",
        sep=",")
    print(dataset.columns.values)
    # columns = dataset.columns.values[1:len(dataset.columns.values)]
    columns = [ 'DayFrame', 'WeekDay',
         'WeekEnd' ,'Clear', 'Cloudy', 'Rain', 'Fog', 'Snow', 'RainBefore', 'Grid_Block',
         'Grid_Col', 'Grid_Row', 'Highway', 'Land_Use_Mode', 'Road_Count']
    # X = columns.shape[0]
    X = len(columns)
    ...
    # print(forecast[0:5])
    # exit()
    print(len(forecast.columns.values))
    # print(forecast.head)
...
filename = "../Graphs & Images/ResultsFromRandomTesting/Second Test - Shuffle/Forecast-for6-10-2019_withRandomPrediction.csv"
modelpath = "../Graphs & Images/ResultsFromRandomTesting/Second Test - Shuffle/model_ORI_Random.h5"
# test["Probability"] = 0
# test["Prediction"] = 0
# test.to_csv("../Excel & CSV Sheets/Grid Oriented Layout Test Files/NegativeSampling/GridFixed/Forecast-for6-10-2019_Done.csv",
#                     sep=",",index=False)
# exit()
afterpred = predict_accidents(test, filename, modelpath)
afterpred.to_csv(filename,index=False)
